    ...
    <div className="space-y-6 sm:space-y-7 animate-fade-in pb-12">
      ...
      <section className="bg-gradient-to-r from-rose-950 via-rose-900 to-rose-950 text-white rounded-3xl p-6 sm:p-8 shadow-sm border border-rose-300/20">
        <div className="flex flex-col md:flex-row items-center justify-between gap-5">
          <div className="space-y-2.5 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/15 text-rose-100 text-xs font-semibold backdrop-blur-xs whitespace-nowrap">
      ...
      <section className="bg-white rounded-3xl border border-rose-200/90 p-5 sm:p-7 shadow-2xs space-y-4">
        <div className="flex items-center gap-2.5">
          <Send className="w-5 h-5 text-rose-900 flex-shrink-0" />
          <div>
            <h3 className="font-display font-bold text-base sm:text-lg text-slate-900">
              Pesan Cepat 1-Klik ke Mas
            </h3>
            <p className="text-xs text-slate-500">
              Pilih template pesan untuk langsung dikirim via WhatsApp tanpa perlu mengetik panjang
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
          {quickMessages.map((msg, idx) => (
            <div
              key={idx}
              className="p-4 sm:p-5 rounded-2xl border border-rose-100/90 bg-rose-50/20 hover:bg-white hover:border-rose-300 hover:shadow-2xs transition-all flex flex-col justify-between space-y-3"
            >
              <div className="space-y-1.5">
                <span className="text-xs font-bold text-slate-900 block truncate">
                  {msg.title}
                </span>
                <p className="font-reading text-xs text-slate-600 leading-relaxed italic line-clamp-3">
                  "{msg.text}"
                </p>
              </div>
      ...
      <section className="bg-white rounded-3xl border border-rose-200/90 p-5 sm:p-6 shadow-2xs space-y-3">
        <h3 className="font-display font-bold text-sm sm:text-base text-slate-900">
          Tulis Pesan Bebas Langsung ke WhatsApp Mas
        </h3>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Tulis pesan apa saja untuk Mas..."
            className="flex-1 px-3.5 py-2.5 text-xs sm:text-sm bg-slate-50 hover:bg-white focus:bg-white rounded-xl border border-slate-200 focus:outline-hidden focus:border-rose-800 transition-all"
          />
          <button
            onClick={() => {
              if (customMsg.trim()) {
            ...
            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-all cursor-pointer flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 active:scale-98"
          >
      ...
      <section className="bg-white rounded-3xl border border-rose-200/90 p-5 sm:p-7 shadow-2xs space-y-4">
        <div className="flex items-center gap-2.5">
          <Building className="w-5 h-5 text-slate-700 flex-shrink-0" />
          <div>
            <h3 className="font-display font-bold text-base sm:text-lg text-slate-900">
              Direktori Kontak Penting Kampus UPI & Bandung
            </h3>
            <p className="text-xs text-slate-500">
              Nomor darurat medis, keamanan kampus FPMIPA, dan kepolisian kota Bandung
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          {campusEmergency.map((item, idx) => (
            <div
              key={idx}
              className="p-4 rounded-2xl bg-slate-50/80 border border-slate-200/80 hover:bg-white hover:border-rose-200 transition-all flex items-center justify-between gap-3"
            >
              <div className="min-w-0">
                <h4 className="text-xs sm:text-sm font-bold text-slate-800 truncate">{item.name}</h4>
                <p className="text-[11px] sm:text-xs text-slate-500 truncate">{item.desc}</p>
              </div>
              <a
                href={`tel:${item.number}`}
                className="px-3.5 py-2 rounded-xl bg-white border border-rose-200 text-rose-900 font-bold text-xs hover:bg-rose-50 flex items-center gap-1.5 flex-shrink-0 whitespace-nowrap shadow-2xs transition-colors"
              >
